        ...
        <AuthLayout>
            <Card>
                ...
                <CardContainer>
                    <CardHeader>
                        <CardHeaderContent>
                            <InputSearch placeholder={"Buscar por cliente"} url={"agendas.index"} date />
                        </CardHeaderContent>
                        <CardHeaderContent>
                                ...
                                {agendas.data.map((agenda: any) => (
                                    <Fragment key={agenda.id}>
                                        <TableRow>
                                            ...
                                            <TableCell>{agenda.cliente.nome}</TableCell>
                                            <TableCell>{moment(agenda.datahora).format("DD/MM/YYYY HH:mm:ss")}</TableCell>
                                            <TableCell>{agenda.tecnico}</TableCell>
                                            <TableCell>{statusAgendaByValue(agenda.status)}</TableCell>
                                            <TableCell className="flex items-center justify-end gap-2">
                                                <EditButton url={route('agendas.edit', agenda.id)} />
                                                <DeleteButton url="agendas.destroy" param={agenda.id} identify={`o agendamento para ${agenda.cliente.nome}`} />
                                            </TableCell>
                                        </TableRow>
                                    </Fragment>
                                ))}
